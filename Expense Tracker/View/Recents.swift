...
struct Recents: View {
    ...
    var body: some View {
        GeometryReader {
                ...
                ScrollView(.vertical){
                    LazyVStack(spacing: 10, pinnedViews: [.sectionHeaders]) {
                        Section {
                            ...
                            CustomSegmentedControl().padding(.bottom ,10)
                                ...
                                SwipeAction(cornerRadius: 15, direction: .trailing) {
                                    ...
                                    Action(tint: .red, icon: "trash.fill") {
                                        withAnimation(.easeInOut ){
                                            print("saml")
                                            sampleTransactions.removeAll(where: { $0 == transaction })
            ...
            .overlay {
                if showFilterView {
                    Expense_Tracker.DateFilterView(start: startDate, end: endDate, submit: { start ,end in
                        startDate = start
                        endDate = end
    ...
    //MAKE THE HEADER TRANSPARENT WHEN SCROLLING
